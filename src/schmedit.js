;(function() {
    ...
    var root = this,
        ...
        Schmeditor = function(el, selector, options) {
            ...
            var _edit = function() {
                if (options.onEdit && options.onEdit(this) === false) return;
                if (_editing) _close();
                _current = this;
                mottle.on(document, CLICK, _maybeCancel);
                mottle.on(_current, KEYPRESS, _keypress);
                mottle.on(_current, BLUR, _maybeCancel);
            ...
            var _close = function(andCommit) {
                ...
                mottle.off(_current, BLUR, _maybeCancel);
                _editing = false;
            ...
            var _maybeCancel = function(e) {
                if (_editing && (e.target.className == null || e.target.className.indexOf("schmediting") == -1 || e.type === "blur"))
                    _close(options.commitOnBlur === true);
            };
